	...
	async close(interaction, client, reason) {
		...
		async function close(res) {
			...
			interaction.channel.send({
				content: client.locales.ticketTranscriptCreated.replace('TRANSCRIPTURL', `<https://teste.cf/${res.data.id}>`),
			}).catch(e => console.log(e));
			await client.db.set(`tickets_${interaction.channel.id}.transcriptURL`, `https://teste.cf/${res.data.id}`);
			const ticket = await client.db.get(`tickets_${interaction.channel.id}`);
			...
			.setDescription(
				...
				.replace('TICKETCOUNT', ticket.id)
				.replace('TRANSCRIPTURL', `https://teste.cf/${res.data.id}`)
		...
		if (Buffer.byteLength(attachment) > 18990000) {
			...
			axios.post('https://teste.cf/upload', {buffer: attachment}, {maxBodyLength: 104857600, maxContentLength: 104857600}).then(res => {
				close(res);
			})
		} else {
			axios.post('https://teste.cf/upload', {buffer: attachment}, {maxBodyLength: 104857600, maxContentLength: 104857600}).then(res => {
